...
void DynamicVoronoi::initializeEmpty(int _sizeX, int _sizeY, bool initGridMap)
{
    ...
    sizeX = _sizeX;
    sizeY = _sizeY;
    dataTemp = new dataCell[sizeX * sizeY];
    data = new dataCell *[sizeX];
    distMap = new float[sizeX * sizeY];
    distMapSQ = new int[sizeX * sizeY];
    for (int x = 0; x < sizeX; x++)
        // data[x] = new dataCell[sizeY];
        data[x] = dataTemp + sizeY * x;
    ...
    for (int x = 0; x < sizeX; x++)
        for (int y = 0; y < sizeY; y++)
            data[x][y] = c,
            distMap[x * sizeY + y] = c.dist,
            distMapSQ[x * sizeY + y] = c.sqdist;
                ...
                for (int dy = -1; dy <= 1; dy++)
                {
                    ...
                    if (nc.obstX != invalidObstData && !nc.needsRaise)
                    {
                        if (!isOccupied(nc.obstX, nc.obstY, data[nc.obstX][nc.obstY]))
                        {
                            ...
                            nc.sqdist = INT_MAX;
                            // data[nx][ny] = nc;
                            distMap[nx * sizeY + ny] = nc.dist;
                            distMapSQ[nx * sizeY + ny] = nc.sqdist;
                        }
                        else
                        {
...
bool DynamicVoronoi::isOccupied(const int &x, const int &y, const dataCell &c)
{
    return (c.obstX == x && c.obstY == y);
}
